...
DimPlot(pbmc.all, group.by='celltype', split.by='stage', pt.size=.01, ncol=2) + 
  ...
  theme(legend.key.size=unit(3,'mm'),
        ...
        legend.position=c(1.2,.3)) #'right')
```

show celltype proportions

```{r Fig_1D,fig.width=1.25,fig.height=2.25}
pbmc.all@meta.data %>% 
  dplyr::group_by(stage,celltype) %>% 
  dplyr::summarise(n=n()) %>% 
...
show celltype proportions again per sample
...
for (dt in c('donor','recipient')) {
  pl <- do.call(rbind,md) %>%
    ...
    labs(y=paste0('fraction CD4/CD8 T cells'),fill='',title=paste0('top10 ',dt,' clonotypes')) +
    facet_grid(.~pair) +
    coord_cartesian(clip='off')
  print(pl)
}
...
```{r Table_S5}
library(openxlsx)
write.xlsx(annotated_md %>%
             dplyr::ungroup() %>%
             dplyr::select(donor,stage,CTaa,anno) %>%
             dplyr::mutate(pair=gsub('^[DR]','p',donor),
                           persistence=ifelse(CTaa %in% Reduce(union,persisting_cts),'persisting','other')) %>%
             dplyr::group_by(pair,stage,CTaa,persistence) %>%
             dplyr::summarise(n=n(),
                              specificity=gsub(',','_',paste0(unique(anno)))) %>%
             dplyr::group_by(pair,stage) %>%
             dplyr::mutate(fraction=n/sum(n),
                           pair=pair_map[pair]) %>%
             dplyr::filter(grepl('CMV|EBV',specificity)),
           '../data/tables/Table_S5.xlsx', sheetName='Table S5', row.names=FALSE)
```
