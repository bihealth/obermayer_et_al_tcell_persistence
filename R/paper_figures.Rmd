...
pair_shapes_1 <- c('p10'=16,'p16'=15,'p26'=18,'p29'=17,'p31'=12,'p36'=13)
...
pair_map <- c('p10'='pair A','p16'='pair B','p26'='pair C','p29'='pair D','p31'='pair E','p36'='pair F')
...
```{r Fig_S2A,fig.width=5.5,fig.height=5}
DimPlot(pbmc.all, group.by='cloneType', split.by='stage', ncol=2) +
  geom_segment(data=anno,
               aes(x=x,y=y,xend=xend,yend=yend),
               arrow=arrow(length=unit(1.5,'mm')),colour='black',size=.25,inherit.aes=FALSE) + 
  geom_text(data=anno_text,
...
- persisting_vs_other: persisting vs other CD8 TEM cells, separately for donors and recipients and excluding pair 31 and 36
...
take_col <- (colSums(counts) > 0) & (colData$celltype %in% c('persisting','other')) & !(colData$pair %in% c('31','36')) & (colData$donor_type=='D')
dds <- DESeqDataSetFromMatrix(countData=counts[take_row,take_col],
                              colData=colData[take_col,,drop=FALSE],
...
take_col <- (colSums(counts) > 0) & (colData$celltype %in% c('persisting','other')) & !(colData$pair %in% c('31','36')) & (colData$donor_type=='R')
dds <- DESeqDataSetFromMatrix(countData=counts[take_row,take_col],
                              colData=colData[take_col,,drop=FALSE],
                              design=~pair+stage+celltype) %>%
  DESeq()
...
write.csv(pseudobulk,file=file.path('..','data','DE','pseudobulk_DE.csv'))
```

load table of "DEGs" of differential genes (at adj. p-value < 0.01) 

```{r pseudobulk_table, cache=FALSE}
pseudobulk <- read.csv(file.path('..','data','DE','pseudobulk_DE.csv'),row.names=1)
...
write.csv(tmods %>%
          ...
          file=file.path('..','data','DE','pseudobulk_tmod.csv'))
```

here's a table "GSEA" of the `tmod` results to excel file

```{r tmod_table}
msig <- tmodImportMSigDB(file.path('..','msigdb_v7.2.xml'))
tmods <- read.csv(file.path('..','data','DE','pseudobulk_tmod.csv'),row.names=1,stringsAsFactors=FALSE)
```
...
PlotORA(scdiffcom,category='LRI') +
  theme_classic() +
  coord_cartesian(clip='off') +
  theme(legend.key.size=unit(3,'mm'),
...
plot phenotype distribution: persisting cells are mostly CD8 TEM (leave out p31)
...
```{r Fig_3D_S3D, fig.width=7,fig.height=2.5}
...
```{r Fig_S3A, fig.width=4.5,fig.height=4.5}
breaks <- c('D10prePBMC','D10postPBMC','R10d180PBMC',
            ...
            'D31postPBMC','R31d90PBMC','R31d180PBMC',
...
labels <- gsub('[DR]10','A_',
               gsub('[DR]16','B_',
                    gsub('[DR]26','C_',
                         gsub('[DR]29','D_',
                              gsub('[DR]31','E_',
                                   gsub('[DR]36','F_',gsub('PBMC','',breaks)))))))
...
pl <- do.call(rbind,md) %>%
  dplyr::mutate(pair=gsub('^[DR]','p',donor)) %>%
  dplyr::filter(predicted.celltype.l1 %in% c('CD4 T','CD8 T'),CTaa %in% Reduce(union,persisting_cts),pair!='p31') %>%
...
take_row <- rowSums(counts) > 0
take_col <- (colSums(counts) > 0) & (colData$celltype %in% c('persisting','other')) & !(colData$pair %in% c('31','36'))
dds <- DESeqDataSetFromMatrix(countData=counts[take_row,take_col],
                              colData=colData[take_col,,drop=FALSE],
                              design=~pair+stage+celltype)
dds <- DESeq(dds)
...
tcells <- subset(pbmc.all, subset=(celltype %in% c('CD4 Naive','NK','CD4 TCM','CD8 TCM','CD8 Naive',
                                                  ...
                                                  'CD4 Proliferating','CD4 CTL')) & !(pair %in% c('p31','p36')))
...
CD8TEM <- subset(pbmc.all, subset=(predicted.celltype.l2=='CD8 TEM') & !(pbmc.all$pair %in% c('p31','p36')))
